...
function Teams() {
  ...
  return (
    <div className="max-h-screen bg-[var(--orange)] text-white p-6 flex-1 flex justify-center">
      <div className="w-full max-w-3xl bg-[var(--blue-light)] backdrop-blur-md border border-[var(--card-border)] rounded-2xl p-6 flex flex-col">
        ...
        {/* Members List */}
        <div className="max-h-[350px] flex flex-col items-center overflow-y-auto no-scrollbar space-y-3 mb-6">
          <AnimatePresence>
            {members.map((member) => (
              <motion.div
                key={member.id}
                ...
                className="flex w-[97%] justify-between items-center bg-[var(--bg-dark)] p-3 rounded-lg border border-gray-700"
              >
                <div>
                  <p>{member.name}</p>
        ...
        <motion.button
          ...
          className="w-64 mx-auto block bg-[var(--orange)] py-2 rounded-lg font-medium"
        >
          + Add Member
        </motion.button>
      </div>
